...
def generate_wandb_rom():
    ...
    write_rom(dst_file_path, 'input_nw_weight.txt', data)
    data = wandb_data['input_network.0.bias'].flatten()
    max_data_of_wandb = max(float(data.abs().max()), max_data_of_wandb)
    write_rom(dst_file_path, 'input_nw_bias.txt', data)
    ...
    for i in range(4):
        if (i == 0):
            ...
            write_rom(dst_file_path, 'node_nw_weight_0_0.txt', data_0)
            write_rom(dst_file_path, 'node_nw_weight_0_1.txt', data_1)
            write_rom(dst_file_path, 'node_nw_weight_0_2.txt', data_2)
        else:
            data = wandb_data['node_network.network.' + str(2 * i) + '.weight'].flatten()
            max_data_of_wandb = max(float(data.abs().max()), max_data_of_wandb)
            write_rom(dst_file_path, 'node_nw_weight_' + str(i) + '.txt', data)
        data = wandb_data['node_network.network.' + str(2 * i) + '.bias'].flatten()
        max_data_of_wandb = max(float(data.abs().max()), max_data_of_wandb)
        write_rom(dst_file_path, 'node_nw_bias_' + str(i) + '.txt', data)
    ...
    for i in range(4):
        if (i == 0):
            ...
            data_0 = data[:, 0 * node_emb_dim: 1 * node_emb_dim].flatten()
            data_1 = data[:, 1 * node_emb_dim: 2 * node_emb_dim].flatten()
            write_rom(dst_file_path, 'edge_nw_weight_0_0.txt', data_0)
            write_rom(dst_file_path, 'edge_nw_weight_0_1.txt', data_1)
        else:
            data = wandb_data['edge_network.network.' + str(2 * i) + '.weight'].flatten()
            max_data_of_wandb = max(float(data.abs().max()), max_data_of_wandb)
            write_rom(dst_file_path, 'edge_nw_weight_' + str(i) + '.txt', data)
        data = wandb_data['edge_network.network.' + str(2 * i) + '.bias'].flatten()
        max_data_of_wandb = max(float(data.abs().max()), max_data_of_wandb)
        write_rom(dst_file_path, 'edge_nw_bias_' + str(i) + '.txt', data)
