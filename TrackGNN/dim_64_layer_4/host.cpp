...
int main(int argc, char** argv)
{
	...
	// Declare event and time variables before the kernel launch loop
	cl::Event event;
	...
	for (int test_idx=0; test_idx<TEST_TIMES; test_idx++)
	{
		...
		OCL_CHECK(err, err = q.enqueueTask(
			kernel_compute_graph, nullptr, &event));
		...
		auto exec_time_ns = nstimeend - nstimestart;
		std::cout << "Kernel execution time: " << exec_time_ns << " ns" << std::endl;
	}
...
static void load_graph()
{
	// Data path.
	# ifdef EMU
	std::string path = "/home/sl3558/TrackGNN/TrackGNN/TrackGNN/prj/input_files/";
	# endif
	# ifdef SIM
	std::string path = "/home/sl3558/TrackGNN/TrackGNN/TrackGNN/prj/input_files/";
	# endif
	//please replace it with your local path
	...
	# endif	
	{
		...
		file_in.open(node_feature_file);
		if (!file_in.is_open()) {
            std::cerr << "Error: Could not open " << node_feature_file << std::endl;
            return;
        }
		else{
